      ...
      <motion.div className="space-y-6" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <section className="rounded-lg border border-border bg-card p-6">
          <div className="grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
            <div>
              <p className="text-sm uppercase tracking-[0.18em] text-muted-foreground">Your intelligence hub</p>
        ...
        <section className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
          <div className="rounded-lg border border-border bg-card p-6">
            ...
            <div className="mt-4 space-y-3">
              {roadmap.map((step, index) => (
                <div key={step} className="flex gap-3 rounded-lg border border-border bg-background-secondary p-3">
                  <span className={index < 2 ? "text-emerald-400" : "text-muted-foreground"}>{index < 2 ? "OK" : "--"}</span>
          ...
          <div className="rounded-lg border border-border bg-card p-6">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              {assetsQuery.isLoading ? [1, 2, 3, 4].map((item) => <div key={item} className="h-32 animate-pulse rounded-lg border border-border bg-background-secondary" />) : recommendedAssets.map((asset) => (
        ...
        <section className="rounded-lg border border-border bg-card p-6">
          ...
          <div className="mt-4 grid gap-3 lg:grid-cols-2">
            {newsQuery.isLoading ? [1, 2, 3, 4].map((item) => <div key={item} className="h-28 animate-pulse rounded-lg border border-border bg-background-secondary" />) : news.map((article) => (
