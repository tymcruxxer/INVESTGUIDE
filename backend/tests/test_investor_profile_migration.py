...
def test_auth_migration_is_registered() -> None:
    """Auth migration follows the Alembic revision chain."""
    migration_path = (
        Path(__file__).resolve().parents[1]
        / "alembic"
        / "versions"
        / "20260626_0005_create_users_and_link_investor_profiles.py"
    )
    spec = spec_from_file_location("auth_migration", migration_path)
    assert spec is not None
    assert spec.loader is not None
    ...
    assert migration.revision == "20260626_0005"
    assert migration.down_revision == "20260626_0004"
    assert callable(migration.upgrade)
    assert callable(migration.downgrade)
